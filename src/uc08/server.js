// ---------------------------------------------------------------------------
// server.js  —  The read-only HTTP API for UC-08's dossiers
// ---------------------------------------------------------------------------
// WHY THIS IS GET-ONLY, DELIBERATELY
// UC-08 is the 🔴 use case with no execution path (see workflow.js's header).
...
// exactly one thing to do with a UC-08 case in the ZAF sidebar: read it.
// ---------------------------------------------------------------------------
...
// rather than re-derived by a UI that would otherwise have to guess "does
// this use case ever show buttons?" from the absence of a POST route.
const ACTIONABILITY = {
  ...
  // of UC-08 itself, not of any request. The sidebar used to infer that
  ...
  tier: "high",
  actionable: false,
  actionableReason: "UC-08 has no execution path — this dossier is for a Tax Ops specialist's manual review only.",
};
 ...
 */
export function createUc08Handler({ dossierStore, allowedOrigin = "*", requireSignedIdentity = false, zafVerifier = null }) {
  return async function handle(req, res) {
    ...
    try {
      ...
      if (req.method === "GET" && isPath(parts, ["api", "dossiers"]) && parts.length === 2) {
        return send(res, 200, { dossiers: dossierStore.list() });
      }
      ...
      console.error(`[uc08-api] ${req.method} ${url.pathname} failed: ${err.stack}`);
...
export function startUc08Server(deps, port = 4023) {
  const server = createServer(createUc08Handler(deps));
